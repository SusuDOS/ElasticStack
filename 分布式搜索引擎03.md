...
功能:按照品牌对文档分组，分组后以文档的score字段计算每一个品牌的最大，最小，平均得分，可以理解为品牌评价。
...
以上有一个缺陷就是将字按照拼音倒排序，会导致拼音相同，但是意义完全不一样的信息被检索出来，因此需要进行以下操作，让分析器和检索器不一致即可解决。

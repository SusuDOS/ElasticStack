...
# 注意全文检索与查询所有的区别.
# 1. 查询所有
# hits表示命中的数据,查询所有数据:默认显示10条,并不会显示全部.
GET /hotel/_search
{
  "query": {
    "match_all": {}
...
但是，当我搜索的内容不是词条，而是多个词语形成的短语时，反而搜索不到:(因为不会分词所以反而收不到.)
...
```json
// 在上海找美华达或者皇冠假日(若是没有这两家也返回其他的，因为这个是should而不是must)并且大于500分数大于45的酒店:41条数据
GET /hotel/_search
{
  "query": {
    "bool": {
...
```json
// 高亮匹配"require_field_match"默认true.
// 高亮字可能在标题，logo，文本中.
